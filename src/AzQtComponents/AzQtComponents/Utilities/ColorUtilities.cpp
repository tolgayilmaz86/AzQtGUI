...
namespace AzQtComponents
{
    QColor AdjustGamma(const QColor& color, qreal gamma)
    {
        ...
        const float r = static_cast<float>(std::pow(rgb.redF(), 1.0 / gamma));
        const float g = static_cast<float>(std::pow(rgb.greenF(), 1.0 / gamma));
        const float b = static_cast<float>(std::pow(rgb.blueF(), 1.0 / gamma));
        const float a = static_cast<float>(rgb.alphaF());
