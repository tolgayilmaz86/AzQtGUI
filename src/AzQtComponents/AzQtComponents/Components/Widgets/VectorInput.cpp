...
void VectorElement::setValue(double newValue)
{
    ...
    bool isCloseMag =
        std::abs(static_cast<float>(m_value) - static_cast<float>(newValue)) <=
        std::numeric_limits<float>::epsilon() *
            std::max(std::max(1.0f, std::abs(static_cast<float>(m_value))),
                     std::abs(static_cast<float>(newValue)));
    if (isCloseMag)
    {
        return;
    }
...
{
    if (!(std::abs(m_value - newValue) <= std::numeric_limits<double>::epsilon()))
    {
        m_value = newValue;
...
void VectorInput::setMinimum(double value)
{
    for (size_t i = 0; i < m_elementCount; ++i)
    {
        ...
        if (!(std::abs(spinBox->minimum() - value) <= DBL_EPSILON))
        {
...
void VectorInput::setMaximum(double value)
{
    for (size_t i = 0; i < m_elementCount; ++i)
    {
        ...
        // Avoid setting this value if its close to the current value, because otherwise Qt will pump events into the queue to redraw/etc.
        if (!(std::abs(spinBox->maximum() - value) <= DBL_EPSILON))
        {
            spinBox->blockSignals(true);
