...
bool ToolButton::drawIndicatorArrowDown(const Style* style, const QStyleOption* option, QPainter* painter, const QWidget* widget, const Config& config)
{
    ...
    const int arrowWidth = static_cast<int>(QStyleHelper::dpiScaled(size.width(), QStyleHelper::dpi(option)));
    const int arrowHeight = static_cast<int>(QStyleHelper::dpiScaled(size.height(), QStyleHelper::dpi(option)));
