...
namespace AzQtComponents
{
    ...
    {
        auto it = std::find_if(m_layers.begin(), m_layers.end(), [layerId](const LayerInfo& layer) -> bool { return layer.m_layerId == layerId; });
    ...
    {
        return std::all_of(m_layers.begin(), m_layers.end(), [](const LayerInfo& entry) -> bool { return entry.m_layer->CanClose(); });
    }
    ...
    {
        auto it = std::find_if(m_layers.begin(), m_layers.end(), [layer](const LayerInfo& entry) -> bool { return entry.m_layer == layer; });
        if (it == m_layers.end())
        {
    ...
    {
        auto it = std::find_if(m_layers.begin(), m_layers.end(), [index](const LayerInfo& entry) -> bool { return entry.m_layerIndex == index; });
        if (it == m_layers.end())
        {
            // Layer has already been removed. This can happen as part of chain of close events, as this would also remove
            //      the layer from the stack.
