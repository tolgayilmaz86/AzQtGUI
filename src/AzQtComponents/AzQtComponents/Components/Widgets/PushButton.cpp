...
bool PushButton::drawPushButtonBevel(const Style* style, const QStyleOption* option, QPainter* painter, const QWidget* widget, const PushButton::Config& config)
{
    ...
    if (!button->isFlat())
    {
        ...
        float radius = isSmallIconButton ? static_cast<float>(config.smallIcon.frame.radius) : static_cast<float>(config.defaultFrame.radius);
...
void PushButton::drawSmallIconFrame(const Style* style, const QStyleOption* option, const QRect& frame, QPainter* painter, const PushButton::Config& config)
{
    ...
    float radius = static_cast<float>(config.smallIcon.frame.radius);
...
{
    const int arrowWidth = static_cast<int>(QStyleHelper::dpiScaled(14, QStyleHelper::dpi(buttonOption)));
    const int arrowHeight = static_cast<int>(QStyleHelper::dpiScaled(8, QStyleHelper::dpi(buttonOption)));
